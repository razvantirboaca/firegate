...
import original from '@shared/i18n';

type Locale = keyof typeof original;
type TranslationKey = keyof (typeof original)['en'];
type TranslationMap = Record<Locale, Record<TranslationKey, string>>;

export default function Codex() {
  const [edited] = useState<Partial<TranslationMap>>(() => {
    try {
  ...
  const [merged, setMerged] = useState<TranslationMap>(() => JSON.parse(JSON.stringify(original)));
  const LANGS = Object.keys(original) as Locale[];
  const allKeys = Object.keys(original.en) as TranslationKey[];

  const getEditedValue = (lang: Locale, key: TranslationKey): string => edited?.[lang]?.[key] || '';

  const getOriginalValue = (lang: Locale, key: TranslationKey): string =>
    original?.[lang]?.[key] || '';
  ...
  const handleAccept = (key: TranslationKey, lang: Locale) => {
  ...
  const handleReject = (key: TranslationKey, lang: Locale) => {
    setMerged((prev) => {
      const updated = {
        ...prev,
        [lang]: {
              ...
              return (
                <tr key={`${key}-${lang}`} className="border-t border-gray-100">
                  ...
                  <td className="p-2 flex gap-2">
                    <button
                      onClick={() => handleAccept(key, lang)}
                      className="bg-green-500 text-white px-2 py-1 rounded text-xs"
                    >
                      ✅ Accept
                    </button>
                    <button
                      onClick={() => handleReject(key, lang)}
